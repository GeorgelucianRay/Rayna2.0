  ...
  return (
    <div className={`${styles.bubble} ${styles.bot}`}>
      <div className={styles.botText}>
        {shown}
        {shown.length < (reply_text || "").length && (
...
// GeoJSON Point din "lat,lon" → [lon, lat]
...
async function findPlacesByName(name, limitPerTable = 5) {
  ...
  for (const t of tables) {
    const { data } = await supabase
      ...
      .limit(limitPerTable);
    (data || []).forEach((row) =>
      all.push({ ...row, _table: t, _mapsUrl: getMapsLinkFromRecord(row) })
    );
  }
  return all;
}
  ...
  const send = async () => {
    ...
    if (awaiting === "anuncio_text") {
      ...
      if (!(role === "admin" || role === "dispecer")) {
        setMessages((m) => [
          ...m,
          { from: "bot", reply_text: "No tienes permiso para actualizar anuncios." },
        ]);
        setAwaiting(null);
        return;
      }
                ...
                onSubmit={async ({ name, url }) => {
                  ...
                  setMessages((mm) => [
                    ...mm,
                    {
                      from: "bot",
                      reply_text: error
                        ? intent.dialog.save_err
                        : tpl(intent.dialog.save_ok, { camera: data }),
                    },
                  ]);
                }}
              />
