...
// --- Iconos ---
const CloseIcon = () => <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="18" x2="6" y1="6" y2="18"></line><line x1="6" x2="18" y1="6" y2="18"></line></svg>;
const ArchiveIcon = () => <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 8v13H3V8"></path><path d="M1 3h22v5H1z"></path><path d="M10 12h4"></path></svg>;
...
function CalculadoraNomina() {
    ...
    // Corecție minoră aici pentru a preveni bug-uri, dar păstrând structura ta
    const defaultPontaj = useMemo(() => ({
        km_start: '', km_final: '', festivos: [], contenedores: 0,
        zilePontaj: Array.from({ length: 31 }, () => ({ desayuno: false, cena: false, procena: false })),
    }), []);
    ...
    useEffect(() => {
        const fetchDrivers = async () => {
            if (profile?.role === 'dispecer') {
                setIsLoading(true);
                const { data, error } = await supabase.from('nomina_perfiles').select('id, user_id, nombre_completo, config_nomina');
                ...
                if (error) {
                    ...
                    const mappedData = data.map(d => ({
                        id: d.user_id, // UUID-ul șoferului
                        numeric_id: d.id, // ID-ul numeric al intrării din nomina_perfiles
    ...
    const handleSaveToArchive = async () => {
        ...
        // --- ADAUGAT: Logica pentru actualizarea kilometrajului ---
        const kmFinal = parseFloat(pontaj.km_final) || 0;
        if (kmFinal > 0) {
            try {
                // Pas 1: Găsim profilul șoferului pentru a afla ce `camion_id` are alocat
                const { data: profileData, error: profileError } = await supabase
                    .from('profiles')
                    .select('camion_id') // Selectăm coloana care face legătura cu tabela `camioane`
                    .eq('id', targetId) // Folosim UUID-ul șoferului pentru a-l găsi
                    .single();

                if (profileError) throw profileError;

                const camionId = profileData?.camion_id;
                
                // Pas 2: Dacă șoferul are un camion alocat, actualizăm kilometrii
                if (camionId) {
                    ...
                    console.log(`Kilometrajul pentru camionul ${camionId} a fost actualizat la ${kmFinal}.`);
                } else {
                    console.log(`Șoferul ${targetId} nu are un camion alocat în profil.`);
                }
            } catch(error) {
                alert(`A apărut o eroare la actualizarea kilometrajului: ${error.message}`);
            }
        }
        // --- SFÂRȘIT LOGICĂ ADĂUGATĂ ---

        // Logica ta originală pentru salvarea în arhivă
    ...
    return (
        <Layout backgroundClassName="calculadora-background">
            <div className={styles.header}>
                ...
                <button type="button" className={styles.archiveButton} onClick={handleViewArchive} disabled={!isReady}>
                ...
                <div className={styles.dispatcherSelector}>
                    ...
                    <select id="sofer-select" onChange={handleSoferSelect} value={soferSelectat || ''}>
                        ...
                        {listaSoferi.map(sofer => (
                            <option key={sofer.numeric_id} value={sofer.id}>{sofer.nombre_completo}</option>
                        ))}
                    </select>
                </div>
            )}
                    ...
                    <div className={styles.column}>
                        ...
                        <div className={styles.card}>
                            ...
                            <div className={styles.festivosSection}>
                                <button type="button" onClick={addFestivo} className={styles.addFestivoButton}>Añadir Festivo / Plus</button>
