...
// --- Componente Helper NOI (pentru logica zilnică) ---
...
function CalculadoraNomina() {
    ...
    // MODIFICARE #1: Structura de date pentru pontaj este acum detaliată pe zi
    ...
    // Folosim logica ta de a încărca șoferii din `nomina_perfiles`
    useEffect(() => {
    ...
    // Logica de încărcare a datelor este acum pentru noul format de pontaj
    useEffect(() => {
    ...
    // MODIFICARE #2: Logica de salvare este acum în acest handler, apelat de butonul din modal
    const handleCloseParteDiario = async () => {
        ...
        if (draftError) {
            ...
            console.log("Ciorna salvată cu succes!");
    ...
    const handleConfigChange = (e) => { const { name, value } = e.target; setConfig(prev => ({ ...prev, [name]: value === '' ? '' : parseFloat(value) || 0 })); };
    const handleSoferSelect = (e) => { setSoferSelectat(e.target.value); setPontaj(defaultPontaj); setRezultat(null); };
    ...
    // MODIFICARE #3: Calculul se face acum pe baza datelor zilnice
    const handleCalculate = () => {
        ...
        setRezultat({ /* ... obiectul rezultat ... */ });
    };
    
    const handleSaveConfig = async () => { /* ... la fel ca în codul tău original ... */ };
    const handleSaveToArchive = async () => { /* ... la fel ca în codul tău original ... */ };
    const handleViewArchive = async () => { /* ... la fel ca în codul tău original ... */ };
    ...
    const renderCalendar = () => {
        ...
        for (let i = 1; i <= daysInMonth; i++) {
            const dayData = pontaj.zilePontaj[i - 1] || {};
