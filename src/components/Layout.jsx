...
// --- Iconițe SVG (Complete și nemodificate) ---
const BellIcon = () => (
...
const Layout = ({ children }) => {
  ...
  const backgroundMap = {
    ...
    '/reparatii': styles.reparatiiBackground, // Ruta nouă adăugată
  };

  // Logica îmbunătățită care gestionează sub-paginile
      ...
      <aside className={styles.navMenu}>
        ...
        <nav className={styles.navLinks}>
          {navLinksData.map(link => (
            <NavLink key={link.id} to={link.id} icon={link.icon} text={link.text} isActive={location.pathname.startsWith(link.id)} onClick={() => setIsMenuOpen(false)} />
          ))}
