...
export default function createGround({ width = 90, depth = 60, color = 0x9aa0a6, abcOffsetX = -10, defOffsetX = 32, abcToDefGap = 16 } = {}) {
  const g = new THREE.Group();

  const groundThickness = 0.5;
  const groundGeo = new THREE.BoxGeometry(width, groundThickness, depth);
  groundGeo.translate(0, -groundThickness / 2, 0);

  const asphaltMaterial = new THREE.MeshStandardMaterial({ color, roughness: 0.95, metalness: 0.02 });
  const earthMaterial = new THREE.MeshStandardMaterial({ color: 0x8b5e3c, roughness: 1.0 });

  const ground = new THREE.Mesh(groundGeo, [
    earthMaterial, earthMaterial, asphaltMaterial, earthMaterial, earthMaterial, earthMaterial,
  ]);
  ground.receiveShadow = true;
  g.add(ground);
  
  const ABC_BASE_Z = -4.0, ABC_ROW_Z = { A: ABC_BASE_Z, B: ABC_BASE_Z - (SLOT_W + 0.10), C: ABC_BASE_Z - 2 * (SLOT_W + 0.10) };
  const START_Z_DEF = ABC_ROW_Z.C + abcToDefGap;
  const ABC_BASE_X = abcOffsetX, DEF_BASE_X = 4.0 + defOffsetX;
  const DEF_COL_X = { D: DEF_BASE_X, E: DEF_BASE_X + (SLOT_W + 0.10), F: DEF_BASE_X + 2 * (SLOT_W + 0.10) };
  ...
  for (const row of ['A', 'B', 'C']) {
    ...
    labR.position.set(ABC_BASE_X + ABC_LABEL_GAP_X, 0.03, z + ABC_LABEL_GAP_Z);
    g.add(labR);
  }
