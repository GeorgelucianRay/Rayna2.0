...
export default function createGround({
  ...
  for (const row of ['A', 'B', 'C']) {
    ...
    // Litera rândului la AMBELE capete (mult mai aproape de bandă)
    const rightEdgeX = ABC_BASE_X;               // capătul din dreapta al benzii
    const leftEdgeX  = ABC_BASE_X - 10 * STEP;   // capătul din stânga al benzii

    const labelRight = makePaintedText(row, { size: 2.0 });
    labelRight.position.set(rightEdgeX + 0.3 * STEP, 0.03, z); // foarte aproape de capăt
    g.add(labelRight);

    const labelLeft = makePaintedText(row, { size: 2.0 });
    labelLeft.position.set(leftEdgeX - 0.3 * STEP, 0.03, z);   // foarte aproape de capăt
    g.add(labelLeft);
  }

  // Numerotare 1..10 din 1 în 1 — pe A și C
  for (let col = 1; col <= 10; col++) {
