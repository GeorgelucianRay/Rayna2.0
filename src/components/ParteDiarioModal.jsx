...
export default function ParteDiarioModal({
    ...
    navigator.geolocation.getCurrentPosition(async (position) => {
      ...
      try {
        ...
        const nearbyLocations = allLocations
          ...
          .filter(loc => loc && loc.distance <= 0.2); // Raza de 200m. Pentru testare, poți mări la 2.0 (2km)

        if (nearbyLocations.length > 0) {
          nearbyLocations.sort((a, b) => a.distance - b.distance);
          handleCursaChange(index, field, nearbyLocations[0].name);
        } else {
  ...
  return (
    <div className={styles.modalOverlay} onClick={onClose}>
      {/* AICI ESTE MAGIA PENTRU A NU SE ÎNCHIDE MODALUL LA CLICK INTERN */}
        ...
        <div className={styles.modalBody}>
          ...
          <div className={styles.parteDiarioSection}>
            ...
            <div className={styles.inputGrid}>
              <div className={styles.inputGroup}> <label>KM Iniciar</label> <input type="number" name="km_iniciar" value={kmIniciar} onChange={onNum}/> </div>
              <div className={styles.inputGroup}> <label>KM Final</label> <input type="number" name="km_final" value={kmFinal} onChange={onNum}/> </div>
            </div>
              ...
              {(data.curse || []).map((cursa, index) => (
                <div key={index} className={styles.cursaItem}>
                  <div className={styles.cursaInputs}>
                    <div className={styles.inputGroup}> <label>Salida</label> <div className={styles.inputWithButton}> <input type="text" value={cursa.start || ''} onChange={(e) => handleCursaChange(index, 'start', e.target.value)} placeholder="Ej: Parking" /> <button onClick={() => handleFindLocationByCoords(index, 'start')} disabled={isLocating}> {isLocating?.index === index && isLocating?.field === 'start' ? '...' : <GpsFixedIcon />} </button> </div> </div>
                    <div className={styles.inputGroup}> <label>Llegada</label> <div className={styles.inputWithButton}> <input type="text" value={cursa.end || ''} onChange={(e) => handleCursaChange(index, 'end', e.target.value)} placeholder="Ej: TCB" /> <button onClick={() => handleFindLocationByCoords(index, 'end')} disabled={isLocating}> {isLocating?.index === index && isLocating?.field === 'end' ? '...' : <GpsFixedIcon />} </button> </div> </div>
          ...
          <div className={styles.parteDiarioSection}>
            ...
            <div className={styles.inputGrid}>
              <div className={styles.inputGroup}> <label>Contenedores barridos</label> <input type="number" name="contenedores" value={data?.contenedores ?? ''} onChange={onNum}/> </div>
              <div className={styles.inputGroup}> <label>Suma festivo/plus (€)</label> <input type="number" name="suma_festivo" value={data?.suma_festivo ?? ''} onChange={onNum}/> </div>
            </div>
          </div>
        </div>
