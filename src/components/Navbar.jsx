...
const Icons = {
  ...
  logout:  (props) => (<svg {...props} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"></path><polyline points="16 17 21 12 16 7"></polyline><line x1="21" x2="9" y1="12" y2="12"></line></svg>),
...
const NavLink = ({ to, text, icon, isActive, onClick }) => {
  const { from, to: toColor } = getAccent(to);
  const inlineStyle = isActive ? ({ '--accent': from, '--accent-strong': toColor }) : undefined;
  const cls = [styles.navLink, isActive ? styles.active : ''].join(' ');
  return (
...
export default function Navbar() {
  ...
  if (!role) return null; // nu afișăm meniul până nu știm rolul

  return (
    <>
      {/* buton header pentru deschidere meniu */}
      <header className={`${styles.header} ${styles.headerTransparent}`}>
        <button onClick={() => setOpen(true)} className={styles.menuButtonHeader}>
          <Icon name="menu" />
        </button>
      </header>
      ...
      <aside className={`${styles.navMenu} ${open ? styles.menuOpen : ''}`}>
        <div className={styles.navHeader}>
          <Link to={HUB_ROUTE} className={styles.hubButton} onClick={() => setOpen(false)} aria-label="Rayna Hub">
            <img src={HUB_IMG} alt="Rayna Hub" className={styles.hubLogo} draggable="false" />
          </Link>
          <div className={styles.headerIcons}>
            {alarms.length > 0 && (
              <button className={styles.notificationBell} onClick={() => setShowNotifs(true)}>
                <Icon name="bell" />
                <span className={styles.notificationBadge}>{alarms.length}</span>
              </button>
            )}
            <button onClick={() => setOpen(false)} className={styles.closeButtonMenu}>
              <Icon name="close" />
            </button>
          </div>
        </div>
        ...
        <div className={styles.modalOverlay}>
          <div className={`${styles.modalContent} ${styles.notificationsModal}`}>
            <div className={styles.modalHeader}>
              ...
              <button onClick={() => setShowNotifs(false)} className="close-button">
                <Icon name="close" />
              </button>
            </div>
