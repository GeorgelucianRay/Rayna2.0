...
/**
 * WakeLock STRICT fără npm:
 * 1) Wake Lock API (Android/Chrome)
 * 2) Fallback video invizibil (iOS/others)
 * 3) Prompt dacă browserul blochează autoplay, ca să obținem un gest direct
 */
...
export default function useWakeLockStrict() {
  ...
  const ensureVideo = () => {
    ...
    // ⚠️ Pune în /public un fișier mic MP4 (1–2s), ex: /tiny.mp4
    // Dacă nu ai încă fișierul, folosește totuși calea — build-ul trece; pe iOS trebuie ulterior să pui fișierul pentru efect.
    v.src = '/tiny.mp4';
  ...
  // Re-acquire când revii în tab / focus dacă era activ
  useEffect(() => {
...
// Un mic overlay pentru confirmare (doar când este necesar)
  ...
  return (
    <div style={back} onClick={onCancel}>
      <div style={card} onClick={e=>e.stopPropagation()}>
        ...
        <p style={p}>El navegador bloqueó el autoplay. Toca “Activar” para mantener la pantalla despierta mientras grabas.</p>
        <div style={row}>
