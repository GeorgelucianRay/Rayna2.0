...
// —— Live overlay (voce ON/OFF controlat intern)
...
export default function RaynaHub() {
  ...
  // textul venit din overlay (voce → text) intră pe același flux ca send()
  async function handleVoiceResultFromOverlay(msg) {
    const textMsg = (msg || "").trim();
    if (!textMsg) return;

    setMessages(m => [...m, { from: "user", text: textMsg }]);

    const { intent, slots } = detectIntent(textMsg, intentsData) || {};
    if (!intent?.type) {
    ...
    if (intent.type === "action") {
      await dispatchAction(intent, slots, textMsg);
      return;
    }
  ...
  async function dispatchAction(intent, slots, userText) {
    ...
    const table = {
      ...
      // ⬇️ INTENȚIA CARE ÎNTREABĂ TIMPUL RĂMAS
    ...
    <div className={styles.shell}>
      ...
      <footer className={styles.inputBar}>
        ...
        <button
          ...
          title="Modul Live (ON/OFF în overlay)"
        >
          ● Live
        </button>
      </footer>
      ...
      <RaynaLiveOverlay
        ...
        onVoiceResult={handleVoiceResultFromOverlay}
