  ...
  return (
    <div className={styles.donutWrap}>
      ...
      <div className={styles.donutLegend}>
        <span>
          <i className={styles.dotLeft} /> Disponibles: {left}
        </span>
        <span>
          <i className={styles.dotUsed} /> Usadas: {usadas}
        </span>
        <span>
          <i className={styles.dotPend} /> Pendientes: {pendientes}
        </span>
        <span>
          <i className={styles.dotTotal} /> Total año: {total}
        </span>
      </div>
    </div>
  );
...
export default function MiPerfilPage() {
  ...
  const vacacionesInfo = useMemo(() => {
    const v = authProfile?.vacaciones_info || null; // jsonb opcional en profiles
    return {
      total: v?.total ?? 23,
      usadas: v?.usadas ?? 0,
  ...
  */
  ...
  const goVacaciones = () => navigate('/vacaciones-standalone'); // <— ruta corectă
  const goCamion = () => authProfile?.camion_id && navigate(`/camion/${authProfile.camion_id}`);
  const goRemolque = () =>
    authProfile?.remorca_id && navigate(`/remorca/${authProfile.remorca_id}`);
