...
/* Widgets (MiniCalendar, Donut) - Neschimbate */
// ... (păstrează aici componentele MiniCalendar și Donut exact cum erau)
function MiniCalendar({ date, marks }) { const y=date.getFullYear(),m=date.getMonth();const first=new Date(y,m,1);const startDay=(first.getDay()+6)%7;const daysInMonth=new Date(y,m+1,0).getDate();const cells=[];for(let i=0;i<startDay;i++)cells.push({blank:!0,key:`b-${i}`});for(let d=1;d<=daysInMonth;d++)cells.push({day:d,key:`d-${d}`});return(<div className={styles.miniCal}><div className={styles.miniCalHead}><span>Lu</span><span>Ma</span><span>Mi</span><span>Ju</span><span>Vi</span><span>Sá</span><span>Do</span></div><div className={styles.miniCalGrid}>{cells.map((c)=>c.blank?<div key={c.key} className={styles.miniBlank}/>:<div key={c.key} className={[styles.miniDay,marks?.has(c.day)?styles.miniHasData:''].join(' ')}>{c.day}</div>)}</div></div>)}
...
export default function MiPerfilPage() {
  ...
  // Stări pentru poza de profil
  const [avatarFile, setAvatarFile] = useState(null);
  const [avatarPreview, setAvatarPreview] = useState(null);
  const fileInputRef = useRef(null);

  // Stări noi pentru cameră
  const [isCameraOpen, setIsCameraOpen] = useState(false);
  const [stream, setStream] = useState(null);
  const videoRef = useRef(null);
  const canvasRef = useRef(null);

  // ... (restul stărilor pentru widget-uri rămân neschimbate)
  ...
  const uploadAvatarToImgur = async (file) => {
    const IMGUR_CLIENT_ID = 'b73752e593255f0'; // <-- Asigură-te că ai aici Client ID-ul tău!
  ...
  const saveProfile = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        camion_id: camionIdToUpdate || null, remorca_id: remorcaIdToUpdate || null, avatar_url: avatarUrl,
      };
  ...
  const startCamera = async () => {
    if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
      try {
        ...
        alert("Nu s-a putut accesa camera. Verifică permisiunile în browser.");
      }
    }
  };
  ...
  return (
    <Layout backgroundClassName="profile-background">
      <div className={styles.page}>
        ...
        {/* Restul paginii (Cards, Widgets) rămâne neschimbat */}
        <div className={styles.cardsGrid}>{/* ... */}</div>
        <div className={styles.widgetsGrid}>{/* ... */}</div>

        {/* ===== MODAL EDITARE PROFIL - REVIZUIT ===== */}
              ...
              <form className={styles.modalBody} onSubmit={saveProfile}>
                <div className={styles.avatarEditSection}>
                  <div className={styles.avatarContainerModal}>
                    <img src={avatarPreview || defaultAvatar} alt="Previzualizare" />
                  </div>
                  <div className={styles.avatarActions}>
                    <button type="button" className={styles.btnSecondary} onClick={() => fileInputRef.current.click()}><UploadIcon /> Subir foto</button>
                    <button type="button" className={styles.btnSecondary} onClick={startCamera}><CameraIcon /> Usar cámara</button>
                ...
                {/* Câmpurile din formular - neschimbate */}
                {/* ... */}
          ...
          <div className={styles.cameraOverlay}>
            <div className={styles.cameraModal}>
              ...
              <div className={styles.cameraControls}>
                <button type="button" className={styles.btnGhost} onClick={stopCamera}>Cancelar</button>
