...
function makeMaterialsCapsOnX_ZLength(brand) {
  ...
  const sideZp = sideT?.clone() ?? null;
  const sideZn = sideT?.clone() ?? null;
  ...
  const mSideZp = new THREE.MeshLambertMaterial({
    color: sideZp ? 0xffffff : 0x9aa0a6,
    map: sideZp,
  });
  const mSideZn = new THREE.MeshLambertMaterial({
    color: sideZn ? 0xffffff : 0x9aa0a6,
    map: sideZn,
  });
  ...
  const mFront = new THREE.MeshLambertMaterial({
    color: front ? 0xffffff : 0xcccccc,
    map: front,
  });
  const mBack = new THREE.MeshLambertMaterial({
    color: back ? 0xffffff : 0xcccccc,
    map: back,
  });
