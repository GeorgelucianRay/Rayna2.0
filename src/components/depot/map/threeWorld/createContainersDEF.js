// DEF: lungime pe X, FRONT/BACK pe ±X
...
function makeMaterialsDEF(brand, L){
  ...
  // DEF: FRONT/BACK pe axa X → (right,left) = (front,back); laterale = ±Z
...
export default function createContainersDEF(data, layout){
  ...
  groups.forEach(g=>{
    ...
    const mats = makeMaterialsDEF(g.brand, g.dims.L);
