...
const TiktokIcon = () => (
  <svg width="24" height="24" viewBox="0 0 28 28" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M20.73 7.44v3.86a.7.7 0 0 1-.7.7h-3.86a.7.7 0 0 1-.7-.7V7.44a.7.7 0 0 1 .7-.7h3.86a.7.7 0 0 1 .7.7Z" stroke="white" strokeWidth="1.4"/>
    <path d="M16.17 11.3v8.84a3.5 3.5 0 1 1-3.5-3.5h3.5Z" stroke="white" strokeWidth="1.4"/>
  </svg>
);
      ...
      <div className={styles.externalLinksContainer}>
        ...
        <div className={styles.socialLinksRow}>
          {socialLinks.map(link => {
            ...
            const iconClass = styles[`${link.icon_type}Link`];
            return (
              <a key={link.id} href={link.url} target="_blank" rel="noopener noreferrer" className={`${styles.socialLink} ${iconClass}`}>
                {icon}
                <span>{link.name}</span>
              </a>
            );
...
export default HomepageDispecer;import React, { useState, useEffect } from 'react';
import { supabase } from '../supabaseClient';
import Layout from './Layout';
