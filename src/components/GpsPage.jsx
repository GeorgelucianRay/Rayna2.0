...
const LocationList = ({ tableName, title }) => {
  ...
  // Funcție reutilizabilă pentru încărcarea locațiilor
  ...
  // Reîncarcă locațiile când se schimbă tabelul, pagina sau termenul de căutare
  useEffect(() => {
    fetchLocations();
  }, [fetchLocations]);
  ...
  const handleAddLocation = async (e) => {
    ...
    if (error) {
      ...
      // reîncarcă locațiile de la prima pagină
      setCurrentPage(1);
      setSearchTerm('');
      fetchLocations(1, '');
      setNewLocation({
  ...
  // Role permise pentru editare (extinde după nevoie)
  const canEdit = profile?.role === 'dispecer' || profile?.role === 'sofer';

  return (
    <>
      {/* Toolbar cu căutare și buton de adăugare */}
      ...
      {selectedLocation && (
        <div className="modal-overlay" onClick={() => setSelectedLocation(null)}>
          <div
            className={`modal-content ${styles.locationModal}`}
            ...
            <form onSubmit={handleAddLocation} className="modal-body">
              ...
              <div className={depotStyles.inputGroup}>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setNewLocation({ ...newLocation, link_maps: e.target.value })
                  }
                />
              </div>
              <div className={depotStyles.inputGroup}>
              ...
              <div className={depotStyles.inputGroup}>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setNewLocation({ ...newLocation, link_foto: e.target.value })
                  }
                />
              </div>
              <div className={`${depotStyles.inputGroup} ${depotStyles.fullWidth}`}>
                ...
                <textarea
                  ...
                  onChange={(e) =>
                    setNewLocation({ ...newLocation, detalles: e.target.value })
                  }
                ></textarea>
              </div>
              <div className="modal-footer">
...
function GpsPage() {
  ...
  return (
    // IMPORTANT: Folosim gpsBackground (camelCase) în loc de gps-background
